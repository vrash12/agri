...
## Local Hostinger account alignment — 2026-09-18

At the owner's request, the four live Baguio/Benguet accounts were synchronized to localhost by email, retaining their production roles and active status. The existing Benguet Super Admin and La Trinidad Municipal Staff were updated; the Baguio Head Agriculturist and the legacy Benguet-office Head Agriculturist were created. Stored password hashes were transferred privately and verified equal; no plaintext password was recovered or recorded. Production was only read.

The owner explicitly chose to match Hostinger's Baguio separation. The existing local Baguio City workspace keeps its numeric ID, name, code, and operational records but now belongs to a separate active Baguio City supervision entry. Benguet administrators no longer have access to that workspace. The legacy Benguet office workspace (`BEN`) was added to match its municipal-head account; this does not grant that account province-wide staff access or create a municipal boundary.
