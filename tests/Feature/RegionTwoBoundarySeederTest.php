...
use Database\Seeders\BatanesMunicipalityBoundarySeeder;
use Database\Seeders\CagayanMunicipalityBoundarySeeder;
use Database\Seeders\IsabelaMunicipalityBoundarySeeder;
use Database\Seeders\NuevaVizcayaMunicipalityBoundarySeeder;
use Database\Seeders\QuirinoMunicipalityBoundarySeeder;
use Database\Seeders\SantiagoCityBoundarySeeder;
...
class RegionTwoBoundarySeederTest extends TestCase
{
    use ReferenceProvinceSchema;

    private const SEEDERS = [
        'Batanes' => BatanesMunicipalityBoundarySeeder::class,
        'Cagayan' => CagayanMunicipalityBoundarySeeder::class,
        'Isabela' => IsabelaMunicipalityBoundarySeeder::class,
        'Nueva Vizcaya' => NuevaVizcayaMunicipalityBoundarySeeder::class,
        'Quirino' => QuirinoMunicipalityBoundarySeeder::class,
        'Santiago City' => SantiagoCityBoundarySeeder::class,
    ];
    ...
    public function test_all_93_boundaries_have_the_correct_supervision_and_attribution(): void
    {
        ...
        foreach (['Batanes' => 6, 'Cagayan' => 29, 'Isabela' => 36, 'Nueva Vizcaya' => 15, 'Quirino' => 6, 'Santiago City' => 1] as $name => $count) {
            ...
            foreach ($municipalities as $municipality) {
                ...
                $this->assertMatchesRegularExpression('/^02[0-9]{8}$/', $audit->metadata['psgc_code']);
                $this->assertMatchesRegularExpression('/^02[0-9]{7}$/', $audit->metadata['legacy_psgc_code']);
            }
        }

        $this->assertSame(93, Municipality::query()->count());
        $this->assertSame(93, MunicipalityBoundary::query()->active()->count());
        $this->assertSame(93, AuditLog::query()->count());
    ...
    {
        $existing = [
            $this->municipality('Tarlac', 'Alcala', 'ALCALA'),
            $this->municipality('Tarlac', 'Quezon', 'QUEZON'),
            $this->municipality('Tarlac', 'Quirino', 'QUIRINO'),
        ];
        ...
        foreach ([
            'Cagayan' => ['Alcala'],
            'Isabela' => ['Quezon', 'Quirino'],
            'Nueva Vizcaya' => ['Quezon'],
        ] as $province => $names) {
            foreach ($names as $name) {
                $municipality = Municipality::query()->where('name', $name.' ('.$province.')')->sole();
                $this->assertSame($this->referenceProvinceId($province), $municipality->province_id);
                $this->assertSame(1, MunicipalityBoundary::query()->active()->where('municipality_id', $municipality->id)->count());
            }
        }
        $this->assertSame(96, Municipality::query()->count());
        $this->assertSame(93, MunicipalityBoundary::query()->active()->count());
    }

    public function test_santiago_city_scope_is_separate_from_isabela_administrators(): void
    {
        app(IsabelaMunicipalityBoundarySeeder::class)->run();
        app(SantiagoCityBoundarySeeder::class)->run();
        $city = Municipality::query()->where('name', 'Santiago City')->sole();
        $this->assertSame($this->referenceProvinceId('Santiago City'), $city->province_id);
        $access = app(MunicipalityAccess::class);
        $isabelaAdministrator = new User([
            'role' => User::ROLE_SUPER_ADMIN, 'province_id' => $this->referenceProvinceId('Isabela'), 'is_active' => true,
        ]);
        $this->assertCount(36, $access->choices($isabelaAdministrator));
        $this->assertNotContains($city->id, $access->choices($isabelaAdministrator)->modelKeys());
        $this->assertSame(36, $access->scope(MunicipalityBoundary::query(), $isabelaAdministrator)->count());
        $this->assertFalse($access->scope(MunicipalityBoundary::query(), $isabelaAdministrator)
            ->where('municipality_id', $city->id)->exists());
        $this->assertFalse($access->scopeMunicipalities(Municipality::query(), $isabelaAdministrator)
        ...
        $this->assertCount(37, $access->choices($this->actor));
        $this->assertSame(1, User::query()->count());
    }

    public function test_an_existing_santiago_workspace_under_isabela_is_not_reassigned(): void
    {
        Province::query()->create(['name' => 'Isabela', 'is_active' => true]);
        $this->municipality('Isabela', 'Santiago City', 'SANTIAGO_CITY');
        $before = $this->databaseState();

        $this->assertImportRejected(SantiagoCityBoundarySeeder::class, 'different province');

        $this->assertSame($before, $this->databaseState());
        $this->assertFalse(Province::query()->where('name', 'Santiago City')->exists());
    }

    public function test_a_late_psgc_match_in_another_province_rolls_back_the_whole_province(): void
    {
        [, $identity] = $this->lastIdentity(CagayanMunicipalityBoundarySeeder::class);
        $this->municipality('Tarlac', 'Existing unrelated office', $identity['psgc_code']);
        $before = $this->databaseState();

        $this->assertImportRejected(CagayanMunicipalityBoundarySeeder::class, 'different province');

        $this->assertSame($before, $this->databaseState());
        $this->assertFalse(Province::query()->where('name', 'Cagayan')->exists());
    }

    public function test_an_inactive_late_workspace_is_preserved_and_prior_imports_are_rolled_back(): void
    {
        Province::query()->create(['name' => 'Nueva Vizcaya', 'is_active' => true]);
        [$name, $identity] = $this->lastIdentity(NuevaVizcayaMunicipalityBoundarySeeder::class);
        $this->municipality('Nueva Vizcaya', $identity['workspace_name'] ?? $name, $identity['code'], false);
        $before = $this->databaseState();

        $this->assertImportRejected(NuevaVizcayaMunicipalityBoundarySeeder::class, 'workspace is inactive');
    ...
    {
        [$name] = $this->lastIdentity(QuirinoMunicipalityBoundarySeeder::class);
        $municipality = $this->municipality('Tarlac', 'Existing approved geofence', 'EXISTING_APPROVED');
        $this->boundaryFromSource($municipality, QuirinoMunicipalityBoundarySeeder::class, $name);
        $before = $this->databaseState();

        $this->assertImportRejected(QuirinoMunicipalityBoundarySeeder::class, 'conflicts with active boundary');

        $this->assertSame($before, $this->databaseState());
        $this->assertFalse(Province::query()->where('name', 'Quirino')->exists());
    }

    public function test_a_changed_existing_reference_is_not_overwritten(): void
    {
        app(BatanesMunicipalityBoundarySeeder::class)->run();
        MunicipalityBoundary::query()->orderByDesc('id')->firstOrFail()
            ->forceFill(['name' => 'Locally reviewed boundary'])->saveQuietly();
        $before = $this->databaseState();

        $this->assertImportRejected(BatanesMunicipalityBoundarySeeder::class, 'already has a different active boundary');
    ...
    {
        $source = new ReflectionClass(BatanesMunicipalityBoundarySeeder::class);
        $before = $this->databaseState();

        try {
            app(ReferenceMunicipalityBoundaryImporter::class)->import(
                'Batanes', $source->getConstant('SOURCE_FILE'), str_repeat('0', 64),
    ...
    {
        $province = Province::query()->create(['name' => 'Isabela', 'is_active' => true]);
        $this->actor->forceFill([
            'role' => User::ROLE_SUPER_ADMIN, 'province_id' => $province->id,
        ])->saveQuietly();
        $before = $this->databaseState();

        $this->assertImportRejected(SantiagoCityBoundarySeeder::class, 'account is required');
    ...
    {
        $province = Province::query()->create(['name' => 'Batanes', 'is_active' => true]);
        $this->actor->forceFill([
            'role' => User::ROLE_SUPER_ADMIN, 'province_id' => $province->id,
        ])->saveQuietly();
