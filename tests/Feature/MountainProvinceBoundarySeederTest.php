...
class MountainProvinceBoundarySeederTest extends TestCase
{
    use ReferenceProvinceSchema;

    private const SEEDERS = ['Mountain Province' => MountainProvinceMunicipalityBoundarySeeder::class];
    ...
    protected function setUp(): void
    {
        ...
        $this->actor = User::withoutEvents(fn () => User::query()->create([
            'name' => 'Mountain Province Boundary Administrator', 'email' => 'mountain-province-admin@example.test',
            'password' => 'unused-test-placeholder', 'role' => User::ROLE_SYSTEM_OWNER, 'is_active' => true,
        ]));
    }

    public function test_all_ten_boundaries_have_the_correct_supervision_and_attribution(): void
    {
        $this->seedRegion();

        foreach (['Mountain Province' => 10] as $name => $count) {
        ...
        $this->assertSame(10, MunicipalityBoundary::query()->active()->count());
    ...
    {
        [, $identity] = $this->lastIdentity(MountainProvinceMunicipalityBoundarySeeder::class);
        $this->municipality('Tarlac', 'Existing unrelated office', $identity['psgc_code']);
        $before = $this->databaseState();

        $this->assertImportRejected(MountainProvinceMunicipalityBoundarySeeder::class, 'different province');

        $this->assertSame($before, $this->databaseState());
        $this->assertFalse(Province::query()->where('name', 'Mountain Province')->exists());
    }

    public function test_an_inactive_late_workspace_is_preserved_and_prior_imports_are_rolled_back(): void
    {
        Province::query()->create(['name' => 'Mountain Province', 'is_active' => true]);
        [$name, $identity] = $this->lastIdentity(MountainProvinceMunicipalityBoundarySeeder::class);
        $this->municipality('Mountain Province', $identity['workspace_name'] ?? $name, $identity['code'], false);
        $before = $this->databaseState();

        $this->assertImportRejected(MountainProvinceMunicipalityBoundarySeeder::class, 'workspace is inactive');
    ...
    {
        [$name] = $this->lastIdentity(MountainProvinceMunicipalityBoundarySeeder::class);
        $municipality = $this->municipality('Tarlac', 'Existing approved geofence', 'EXISTING_APPROVED');
        $this->boundaryFromSource($municipality, MountainProvinceMunicipalityBoundarySeeder::class, $name);
        $before = $this->databaseState();

        $this->assertImportRejected(MountainProvinceMunicipalityBoundarySeeder::class, 'conflicts with active boundary');
    ...
    {
        app(MountainProvinceMunicipalityBoundarySeeder::class)->run();
        MunicipalityBoundary::query()->orderByDesc('id')->firstOrFail()
            ->forceFill(['name' => 'Locally reviewed boundary'])->saveQuietly();
        $before = $this->databaseState();

        $this->assertImportRejected(MountainProvinceMunicipalityBoundarySeeder::class, 'already has a different active boundary');
    ...
    {
        $source = new ReflectionClass(MountainProvinceMunicipalityBoundarySeeder::class);
        $before = $this->databaseState();

        try {
            app(ReferenceMunicipalityBoundaryImporter::class)->import(
                'Mountain Province', $source->getConstant('SOURCE_FILE'), str_repeat('0', 64),
    ...
    public function test_a_provincial_administrator_cannot_import_another_province(): void
    {
        ...
        $this->assertImportRejected(MountainProvinceMunicipalityBoundarySeeder::class, 'account is required');

        $this->assertSame($before, $this->databaseState());
        $this->assertFalse(Province::query()->where('name', 'Mountain Province')->exists());
    }

    public function test_bontoc_name_and_province_access_preserve_another_provinces_workspace(): void
    {
        Province::query()->create(['name' => 'Southern Leyte', 'is_active' => true]);
        $other = $this->municipality('Southern Leyte', 'Bontoc', 'BONTOC');
        $before = $other->getAttributes();
        $this->seedRegion();
        $this->assertSame($before, $other->fresh()->getAttributes());
        $this->assertSame(0, MunicipalityBoundary::where('municipality_id', $other->id)->count());
        $bontoc = Municipality::where('name', 'Bontoc (Mountain Province)')->sole();
        $province = Province::where('name', 'Mountain Province')->sole();
        $this->assertSame($province->id, $bontoc->province_id);
        $access = app(MunicipalityAccess::class);
        $admin = new User(['role' => User::ROLE_SUPER_ADMIN, 'province_id' => $province->id, 'is_active' => true]);
        $this->assertCount(10, $access->choices($admin));
        $this->assertNotContains($other->id, $access->choices($admin)->modelKeys());
        $this->assertSame(10, $access->scope(MunicipalityBoundary::query(), $admin)->count());
        $outsider = new User(['role' => User::ROLE_SUPER_ADMIN, 'province_id' => $other->province_id, 'is_active' => true]);
        $this->assertSame(0, $access->scope(MunicipalityBoundary::query(), $outsider)->count());
    }
