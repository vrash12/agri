...
final class OperationsViewFixtures
{
    ...
    public static function data(string $view): array
    {
        ...
        return [
            ...
            'farmerOptions' => collect([$farmer, $farmer2])->map(fn ($person) => [
                'value' => (string) $person->id,
                'label' => trim($person->last_name.', '.$person->first_name).' — '.($person->ffrs ?: 'Not assigned'),
                'municipality_id' => (int) $person->municipality_id,
                'dataset' => [
                    'name' => trim($person->last_name.', '.$person->first_name),
                    'ffrs' => $person->ffrs ?: 'Not assigned',
                    'municipalityId' => (string) $person->municipality_id,
                    'location' => $person->farm_location ?: 'Not recorded',
                    'municipality' => $person->farm_municipality ?: 'Not recorded',
                    'province' => $person->farm_province ?: 'Not recorded',
                    'area' => 'Not recorded',
                    'contact' => $person->contact_number ?: 'Not recorded',
                    'tags' => 'None',
                ],
            ]),
            'browsingAll' => false, 'browseUrl' => '/preview?browse=1',
