...
@once
  @push('styles')
    <style>
      .farmer-workspace-control{position:relative;top:auto;z-index:2;display:grid;grid-template-columns:minmax(180px,.72fr) minmax(310px,1.2fr) minmax(310px,1fr);align-items:stretch;gap:10px;min-width:0;padding:10px;border:1px solid #d3e1d7;border-radius:14px;background:rgba(255,255,255,.96);box-shadow:0 8px 26px rgba(20,52,31,.09);backdrop-filter:blur(14px)}
      ...
      .farmer-workspace-control{border-radius:12px;box-shadow:none;background:#fff;backdrop-filter:none;grid-template-columns:minmax(0,1fr) minmax(260px,1fr)}
      .farmer-workspace-control:before,.workspace-identity,.workspace-scope-summary{display:none}
      .workspace-view-switch{grid-column:auto}
      .workspace-select{height:44px;font-size:16px;border-color:var(--ui-control-border,#7a9182)}
      .workspace-apply{height:44px;font-size:14px;font-weight:500;background:var(--ui-primary,#236344)}
      .workspace-scope-head label,.workspace-view-link strong{font-size:14px;font-weight:500}
      .workspace-view-link{min-height:44px}
      .workspace-view-link.is-active:after{display:none}
      .workspace-view-link small,.workspace-scope-head small{display:none}
      .workspace-select:focus-visible,.workspace-apply:focus-visible,.workspace-view-link:focus-visible{outline:3px solid var(--ui-focus,#236344);outline-offset:3px}
      .workspace-view-link.is-active{color:var(--ui-primary);background:var(--ui-accent-soft);border-color:var(--ui-primary);box-shadow:none}
      @media(max-width:720px){.farmer-workspace-control{grid-template-columns:1fr}}
    </style>
  @endpush
        ...
        if (mapTarget && 'IntersectionObserver' in window) {
          new IntersectionObserver(function (entries) {
            entries.forEach(function (entry) {
              if (entry.isIntersecting && mapTarget.closest('details')?.open) activate('map');
...
<nav class="farmer-workspace-control" aria-label="Farmers workspace controls" data-farmer-workspace-control>
  ...
  @if($workspaceCanChoose)
    <form class="workspace-scope-form" method="GET" action="{{ route('farmers.index') }}#farmerDirectory" data-workspace-scope-form>
      ...
      <small class="workspace-scope-help">Applies to the registry, parcel map, and weather.</small>
    </form>
  @else
