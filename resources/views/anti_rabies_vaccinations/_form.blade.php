...
@push('styles')
<style>
  .animal-owner-lookup{display:none;margin-top:12px;padding:13px;border:1px solid #b9d8c4;border-radius:10px;background:#f1f8f3}.animal-owner-lookup.is-visible{display:block}.animal-owner-lookup-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.animal-owner-lookup h3{margin:0;color:var(--module-green);font-size:12px;font-weight:700}.animal-owner-lookup p{margin:3px 0 0;color:var(--module-muted);font-size:12px}.animal-owner-actions{display:flex;justify-content:flex-end;gap:7px;margin-top:10px;flex-wrap:wrap}
  .animal-service-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.animal-service-option{position:relative;min-width:0}.animal-service-option input{position:absolute;opacity:0;pointer-events:none}.animal-service-option label{display:grid;grid-template-columns:20px minmax(0,1fr);gap:4px 10px;height:100%;min-height:76px;box-sizing:border-box;margin:0;padding:14px;border:1px solid var(--ui-control-border);border-radius:var(--ui-radius-control);background:var(--ui-surface);cursor:pointer}.animal-service-option label:hover{border-color:var(--ui-primary);background:var(--ui-surface-subtle)}.animal-service-option input:focus-visible+label{outline:3px solid var(--ui-focus);outline-offset:2px}.animal-service-option input:checked+label{color:var(--module-green);border-color:var(--ui-primary);background:var(--ui-primary-soft)}.animal-service-indicator{width:18px;height:18px;box-sizing:border-box;margin-top:2px;border:1px solid var(--ui-control-border);border-radius:50%;background:var(--ui-surface)}.animal-service-option input:checked+label .animal-service-indicator{border:5px solid var(--ui-primary)}.animal-service-option label strong{font-size:14px;font-weight:500;line-height:1.5;overflow-wrap:anywhere}.animal-service-option label small{grid-column:2;color:var(--module-muted);font-size:12px;line-height:1.5}
  .animal-form-callout{display:flex;gap:10px;align-items:flex-start;margin-bottom:14px;padding:12px;border:1px solid #cbdde9;border-radius:9px;color:#2d5165;background:#f3f8fb}.animal-form-callout strong,.animal-form-callout span{display:block}.animal-form-callout strong{font-size:12px}.animal-form-callout span{margin-top:2px;font-size:12px;line-height:1.4}.animal-form-callout i{width:24px;height:24px;display:grid;place-items:center;flex:0 0 auto;border-radius:7px;color:#fff;background:#3978b5;font-style:normal;font-size:12px;font-weight:700}.animal-field-error{display:block;margin-top:5px;color:#a43d38;font-size:12px;font-weight:700}.module-input.is-invalid{border-color:#cb625d;background:#fffafa}.animal-form-aside-value{display:block;margin-top:5px;color:var(--module-ink);font-size:14px;font-weight:700;overflow-wrap:anywhere}.animal-form-aside-service{display:inline-flex;margin-top:8px;padding:5px 8px;border-radius:999px;color:#17643a;background:#e7f4eb;font-size:12px;font-weight:700;text-transform:none;letter-spacing:.04em}
...
<div class="module-form-shell">
  <div class="module-form-main">
    ...
    <section class="module-form-section">
      <div class="module-form-section-head"><span class="module-step">2</span><div><h2 id="animalServiceHeading">Select the service</h2><p>Choose one service for an individual animal or a group treated together.</p></div></div>
      <div class="module-form-body"><div class="animal-service-grid" role="radiogroup" aria-labelledby="animalServiceHeading">
        @foreach(($serviceTypeOptions ?? []) as $type => $label)
          @php $description = ['vaccination'=>'Disease prevention, including anti-rabies','deworming'=>'Internal or external parasite control','vitamins'=>'Vitamins, minerals, and supplements','treatment'=>'Illness, injury, or supportive care'][$type] ?? 'Animal-health service'; @endphp
          <div class="animal-service-option"><input type="radio" id="service_type_{{ $type }}" name="service_type" value="{{ $type }}" @checked($selectedServiceType === $type) required><label for="service_type_{{ $type }}"><span class="animal-service-indicator" aria-hidden="true"></span><strong>{{ $label }}</strong><small>{{ $description }}</small></label></div>
        @endforeach
      </div>@error('service_type')<span class="animal-field-error">{{ $message }}</span>@enderror</div>
    </section>

    <section class="module-form-section">
      <div class="module-form-section-head"><span class="module-step">3</span><div><h2>Animal and service details</h2><p>Enter the species, number served, product or treatment, and service date.</p></div></div>
      <div class="module-form-body">
        <div class="module-form-grid">
      ...
      <details class="module-more" id="animalIdentification" @if(collect(['birthday', 'pet_breed', 'pet_color'])->contains(fn ($key) => filled($value($key)) || $errors->has($key))) open @endif>
        <summary>Additional animal and owner details <span class="module-hint">Optional</span></summary>
        <div class="module-more-content"><div class="module-form-grid">
          <div class="module-form-field"><label for="birthday">Owner birthday <span class="module-hint">Optional</span></label><input class="module-input @error('birthday') is-invalid @enderror" type="date" id="birthday" name="birthday" value="{{ $dateValue('birthday') }}" max="{{ $today }}" aria-describedby="birthday_error">@error('birthday')<span class="animal-field-error" id="birthday_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field"><label for="pet_breed">Breed / strain</label><input class="module-input @error('pet_breed') is-invalid @enderror" id="pet_breed" name="pet_breed" list="animalBreedSuggestions" value="{{ $value('pet_breed') }}" maxlength="120" placeholder="Select species first or type a breed" aria-describedby="pet_breed_error"><datalist id="animalBreedSuggestions"></datalist>@error('pet_breed')<span class="animal-field-error" id="pet_breed_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field"><label for="pet_color">Color / markings</label><input class="module-input @error('pet_color') is-invalid @enderror" id="pet_color" name="pet_color" value="{{ $value('pet_color') }}" maxlength="80" placeholder="Optional identifying details" aria-describedby="pet_color_error">@error('pet_color')<span class="animal-field-error" id="pet_color_error">{{ $message }}</span>@enderror</div>
        </div></div>
      </details>
      <details class="module-more" id="animalTreatment" @if(collect(['dosage', 'administration_route', 'diagnosis', 'next_service_date', 'treatment_notes'])->contains(fn ($key) => filled($value($key)) || $errors->has($key))) open @endif>
        <summary>Dosage, treatment and follow-up <span class="module-hint">Optional</span></summary>
        <div class="module-more-content"><div class="module-form-grid">
          <div class="module-form-field"><label for="dosage">Dosage / amount</label><input class="module-input @error('dosage') is-invalid @enderror" id="dosage" name="dosage" value="{{ $value('dosage') }}" maxlength="120" placeholder="e.g. 1 mL/head or 10 g/L water" aria-describedby="dosage_error">@error('dosage')<span class="animal-field-error" id="dosage_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field"><label for="administration_route">Administration route</label><select class="module-input js-select @error('administration_route') is-invalid @enderror" id="administration_route" name="administration_route" aria-describedby="administration_route_error"><option value="">Select route</option>@foreach($administrationRoutes as $route)<option value="{{ $route }}" @selected($value('administration_route') === $route)>{{ $route }}</option>@endforeach</select>@error('administration_route')<span class="animal-field-error" id="administration_route_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field module-form-field-full"><label for="diagnosis">Diagnosis, reason, or indication</label><input class="module-input @error('diagnosis') is-invalid @enderror" id="diagnosis" name="diagnosis" value="{{ $value('diagnosis') }}" maxlength="255" placeholder="e.g. Routine deworming, vitamin support, wound, respiratory signs" aria-describedby="diagnosis_error">@error('diagnosis')<span class="animal-field-error" id="diagnosis_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field"><label for="next_service_date">Next service / follow-up date</label><input class="module-input @error('next_service_date') is-invalid @enderror" type="date" id="next_service_date" name="next_service_date" value="{{ $dateValue('next_service_date') }}" min="{{ $dateValue('vaccination_date', $today) }}" aria-describedby="next_service_date_error">@error('next_service_date')<span class="animal-field-error" id="next_service_date_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field module-form-field-full"><label for="treatment_notes">Service notes</label><textarea class="module-input @error('treatment_notes') is-invalid @enderror" id="treatment_notes" name="treatment_notes" rows="3" maxlength="3000" placeholder="Instructions, withdrawal period, observed condition, response, batch number, or follow-up notes" aria-describedby="treatment_notes_error">{{ $value('treatment_notes') }}</textarea>@error('treatment_notes')<span class="animal-field-error" id="treatment_notes_error">{{ $message }}</span>@enderror</div>
        </div></div>
      </details>
