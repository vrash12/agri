...
<div class="module-page fp-staff">
    ...
    @if($activationCode)
        <section class="fp-panel fp-activation-result" aria-labelledby="activation-result-title"><h2 id="activation-result-title">Activation code issued</h2><p>Give these details privately to the verified farmer. This code is shown only now and can be used once within 24 hours.</p><dl class="fp-details"><div><dt>AgriGOV ID</dt><dd>{{ $account->login_id }}</dd></div><div><dt>Activation page</dt><dd><a href="{{ route('farmer-portal.activate') }}">{{ route('farmer-portal.activate') }}</a></dd></div></dl><label class="fp-code-label" for="issued_activation_code">One-time activation code</label><div class="fp-password fp-code"><input class="module-input" id="issued_activation_code" type="password" readonly value="{{ $activationCode }}" autocomplete="off" spellcheck="false"><button class="fp-reveal" type="button" data-password-toggle="issued_activation_code" aria-controls="issued_activation_code" aria-pressed="false" hidden>Show</button></div><p class="fp-small">The farmer chooses their own password. Do not record this code in notes, screenshots, or shared files.</p></section>
    @endif
    <section class="fp-panel"><h2>Account status</h2><dl class="fp-details"><div><dt>Status</dt><dd>{{ ! $account ? 'Not issued' : (! $account->is_active ? 'Disabled' : ($account->activated_at ? 'Active' : 'Awaiting activation')) }}</dd></div><div><dt>AgriGOV ID</dt><dd>{{ $farmer->agri_gov_id }}</dd></div>@if($account?->activation_expires_at)<div><dt>Activation expires</dt><dd>{{ \App\Support\LocalTime::fromUtc($account->activation_expires_at)?->format('M j, Y g:i A') }} ({{ \App\Support\LocalTime::timezone() }})</dd></div>@endif</dl><p class="fp-small">Every registered farmer has an AgriGOV ID. Portal sign-in requires an issued activation code and a chosen password. The farmer can then view only their linked profile, parcels, and assistance.</p></section>
