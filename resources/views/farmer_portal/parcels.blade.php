...
<form method="GET" class="fp-year-filter"><label for="year">Crop year</label><input class="module-input" id="year" name="year" type="number" min="1900" max="{{ now()->year + 1 }}" value="{{ $cropYear }}" required><button class="module-button" type="submit">Show season records</button></form>
@if($plots->isEmpty())
    <section class="fp-empty"><h2>No farm parcels recorded yet</h2><p>Ask your agriculture office to check or map the parcels linked to your farmer record.</p></section>
@else
