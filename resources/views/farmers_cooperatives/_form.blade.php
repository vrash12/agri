...
<div class="module-form-shell">
  <div class="module-form-main">
    <section class="module-form-section">
      ...
      <div class="module-form-body">
        <div class="module-form-grid">
          ...
          <div class="module-form-field"><label for="chairperson">Chairperson</label><input class="module-input" id="chairperson" type="text" name="chairperson" value="{{ $value('chairperson') }}" maxlength="255" placeholder="Full name" autocomplete="name" aria-describedby="chairperson_error">@error('chairperson')<span class="module-hint module-required" id="chairperson_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field"><label for="contact_number">Contact number</label><input class="module-input" id="contact_number" type="tel" name="contact_number" value="{{ $value('contact_number') }}" maxlength="50" placeholder="e.g. 09XX XXX XXXX" autocomplete="tel" aria-describedby="contact_number_error">@error('contact_number')<span class="module-hint module-required" id="contact_number_error">{{ $message }}</span>@enderror</div>
          <div class="module-form-field module-form-field-full"><label for="address">Office or meeting address</label><input class="module-input" id="address" type="text" name="address" value="{{ $value('address') }}" maxlength="255" placeholder="Barangay, municipality, province" autocomplete="street-address" aria-describedby="address_error">@error('address')<span class="module-hint module-required" id="address_error">{{ $message }}</span>@enderror</div>
        </div>
      </div>
    </section>
    ...
    <details class="module-more" @if(filled($value('description')) || $errors->has('description')) open @endif>
      ...
      <div class="module-form-body"><div class="module-form-field module-form-field-full"><label for="description">Description</label><textarea class="module-input" id="description" name="description" rows="5" placeholder="Services, commodities, coverage area, or other useful notes" aria-describedby="description_error">{{ $value('description') }}</textarea>@error('description')<span class="module-hint module-required" id="description_error">{{ $message }}</span>@enderror<div class="module-hint">Keep this concise and useful for staff handling membership and reports.</div></div></div>
    </details>
