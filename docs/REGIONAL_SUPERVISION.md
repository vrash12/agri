...
`BicolBoundarySeeder` supplies 114 explicit planning/reference municipality and city boundaries across Albay, Camarines Norte, Camarines Sur, Catanduanes, Masbate, Naga City and Sorsogon. `region-access:configure --owner=<id> --region=region5` links those scopes. Naga City is its own scope and is excluded from Camarines Sur provincial choices; Iriga City remains in Camarines Sur. The atomic, audited, idempotent configuration creates no accounts. Hostinger verified all 114 choices and existing-row/account preservation in runtime commit `aabace9` on September 23, 2026. See [BICOL_BOUNDARY_SOURCES.md](BICOL_BOUNDARY_SOURCES.md).
