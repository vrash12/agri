...
The farmer digital/printable ID now uses a DA green-and-gold gradient card design with the Republic of the Philippines coat of arms, Department of Agriculture logo, AgriGOV identity, and a QR panel for the read-only parcel map. The front carries the identity details; the back uses the AgriGOV wordmark without repeating the front header text. The same artwork is used by on-screen, print, and PNG export views. Farmer fields, parcel-address safeguards, role-scoped access, and the statement that the card is not a national government ID remain unchanged. This card redesign is deployed in runtime `d796071`. See [AGRIGOV_FARMER_IDS.md](docs/AGRIGOV_FARMER_IDS.md).
...
Latest interface release: Hostinger installed `bd75ced` on September 24, 2026. Farmers opens directly to the visible directory and parcel map with role-aware region/province/municipality filters, and office/farmer sign-in share one clear audience switch. Homepage collages use Previous/Next arrows. CALABARZON has one active Regional Head plus active provincial administrator and staff accounts for Cavite, Batangas and Laguna; each account's live sign-in and assigned municipality choices were checked.
...
An explicit System Owner demonstration import can populate four clearly labeled sample farmers in each of three Region I municipalities, three hypothetical plots per farmer and sample rice distribution: 12 farmers, 36 plots, 12 releases and three batches (480 kg total). The import preserves existing records and accounts, issues no farmer sign-ins, and rejects duplicates or changed samples. A separately authorized September 24 testing operation enabled portal sign-in for exactly these 12 synthetic farmers; all 12 live sign-ins were verified. **These samples appear in dashboard totals and do not represent actual farmers, surveyed land or seed delivery.** This was deployed to Hostinger on September 24, 2026 for Bacarra, Narvacan and Bacnotan after a verified private backup. See `docs/REGION_I_SAMPLE_DATA.md` for workspaces and verification.
...
Authorized agriculture staff issue access through **Farmer profile/history → Farmer portal access**, after verifying identity. A single-use code expires in 24 hours; the farmer chooses their own password. A stable AgriGOV login ID always works; a recorded RSBSA number is accepted for sign-in only when unique. Birthdays cannot be used to activate or recover accounts. This does not connect to a central RSBSA login service. The September 24 release shares the office and farmer login layout, with a clear switch between both. Activation prompts are removed from the login screen while staff-assisted activation remains available at its separate route.

Staff can reissue access for recovery or disable it. Both revoke old access. There is no automatic portal account creation, SMS delivery, public registration, farmer editing, assistance application, correction-request queue or digital-ID download in this first version. Contact the agriculture office for corrections. Every saved farmer has an AgriGOV ID; issuing portal access enables activation with that same ID. The primary-ID display/search update was installed on Hostinger in `bd75ced`; see [AgriGOV farmer IDs](docs/AGRIGOV_FARMER_IDS.md).

The read-only satellite map opens with all owned parcel boundaries fitted into one map. A farmer can choose a parcel to see its area and selected-year crop records, while the existing individual parcel map links remain available and the records disclosure supports map failures. Collection requests use owned, municipality-scoped cursor pages with bounded geometry and clear office-review messages for invalid records. The portal has mobile layouts, pagination, missing-record messages, sign-out and inactivity expiration. Installation requires one additive account-table migration. See `docs/FARMER_PORTAL.md` for activation and deployment details. The overview cards, richer seed-release details, bounded `/farmer-portal/harvests` history and map-first My Farm enhancement were deployed on Hostinger through GitHub in runtime `c07e78f` on September 24. Existing records and accounts were preserved. An approved real-farmer pilot remains pending; see `docs/FARMER_PORTAL_DEPLOYMENT_2026_09_20.md`.
