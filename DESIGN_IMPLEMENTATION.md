...
## Farmer My Farm map — September 24, 2026 (local)
...
Verification: 34 portal PHP tests / 424 assertions and 11 JavaScript tests pass, alongside Pint, syntax, route, Blade and whitespace checks. A real Google satellite map displayed three synthetic owned parcels in a disposable local preview; desktop and 390px phone layouts and selection details were reviewed. The missing-map configuration opened readable records and retained parcel selection. No browser warnings/errors were captured in the map preview. This follow-up is local, requires no migration or new configuration, and has not been pushed or deployed. See `docs/FARMER_PORTAL.md`.
