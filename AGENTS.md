...
`FarmerPortalRecords` supplies read-only My Profile, the overview, My Farm, My Assistance and My Harvests. Ownership comes from the authenticated account, not input. Releases and harvests must match both farmer and municipality foreign keys, never an identity snapshot. Parcels paginate at 10, releases at 15 and `/farmer-portal/harvests` paginates at 15 with an optional harvest-year filter. The overview uses bounded aggregate counts, previews at most five recent releases/harvests and three parcels, and keeps quantity summaries separate by recorded type and unit. Seasonal crops match the selected year, owned parcel and municipality. Harvest parcel names are loaded only when the parcel is also owned by the signed-in farmer. Private map HTML has no geometry; `/farmer-portal/parcels/{plot}/geometry` returns one owned parcel capped at 10,000 points and 1 MB stored geometry. Google Maps loads only on request and has loading/retry states. No other farmers, office directory, operational editing, or applications are available.
