...
The farmer registry card uses `public/css/farmer-id-card.css`, `public/images/branding/farmer-card-background.svg`, `public/images/branding/philippines-coat-of-arms.png`, and the shared AgriGOV wordmark for its green/gold DA-aligned visual treatment. The front carries farmer identity details; the back uses the wordmark without duplicating that header. Keep the Philippine emblem attribution in the card view and preserve the disclaimer that the card is not a national government ID. Screen, print and canvas PNG exports must use the same source artwork and must fail safely if artwork cannot load; farmer data, QR destination, role scope and parcel-address safeguards must not change. See [docs/AGRIGOV_FARMER_IDS.md](docs/AGRIGOV_FARMER_IDS.md).
...
Public forms at `/farmer-portal/login` and `/farmer-portal/activate` retain CSRF and route throttles. The login screen is deliberately login-only: it shows the farmer ID/password form and a switch to Office sign in, while activation remains a separate staff-assisted setup route and is not promoted from the login form. Passwords require 15 characters, confirmation, at most 72 bytes, bcrypt hashing, and the existing bounded breached-password verifier. Five authentication failures lock the identifier/IP and account for five minutes; failed-event audit volume is capped. `EnsureFarmerPortalSession` refreshes state, checks active account/ownership/municipality/province, password/activation, session version and idle timeout, authorizes the self-account policy, and never switches the office guard to farmer. Private responses are non-storable and load the history-restoration guard. Portal audit URLs omit query strings. No remember-me, birthday authentication, SMS, or public recovery lookup exists.

`FarmerPortalRecords` supplies read-only My Profile, the overview, My Farm, My Assistance and My Harvests. Ownership comes from the authenticated account, not input. Releases and harvests must match both farmer and municipality foreign keys, never an identity snapshot. Parcels paginate at 10 for the records disclosure, releases at 15 and `/farmer-portal/harvests` paginates at 15 with an optional harvest-year filter. The overview uses bounded aggregate counts, previews at most five recent releases/harvests and three parcels, and keeps quantity summaries separate by recorded type and unit. Seasonal crops match the selected year, owned parcel and municipality. Harvest parcel names are loaded only when the parcel is also owned by the signed-in farmer. Private map HTML has no geometry; `/farmer-portal/parcels/{plot}/geometry` remains available for one owned parcel, while `/farmer-portal/parcels/geometry` returns all owned boundaries through municipality-scoped cursor pages of 20, with each response capped at 1 MB of stored geometry and each ring capped at 10,000 points. Google Maps loads on the My Farm page with loading/retry states, fit-all behavior and a map failure fallback. No other farmers, office directory, operational editing, or applications are available.
...
The Farmers directory uses `FarmerWorkspace` to open immediately with paginated farmer records and an open parcel map across active authorized municipalities. System Owners can filter by region; Regional Heads can filter by province or separate city scope within their assigned region. Optional municipality filtering and existing municipality bookmarks remain available. The same validated municipality IDs constrain registry aggregates, map totals, farmer lookup, parcels and active outlines; registry-only filters still leave the map unchanged. Geography changes clear stale children, and clearing filters never widens account permissions. The outline layer defaults to 1000 features (`MAP_MAX_BOUNDARIES_PER_REQUEST`, hard maximum 1000), so the active broad outline set is included by default; a lower deployment setting still produces an explicit partial-layer notice. Parcels retain their separate existing cap. Unassigned regions remain an explicit group without geographic reassignment. See `docs/FARMER_WORKSPACE_AND_COVERAGE.md` for local verification and deployment status.
